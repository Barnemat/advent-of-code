...
const getShortestDistances = (map: DistanceMap): [string, string, number][] => {
  ...
  return allDistances.sort((a, b) => a[2] - b[2]);
};
...
const run = (input: string) => {
  ...
  const shortestDistances = getShortestDistances(distanceMap);
  ...
  for (const [node1, node2] of shortestDistances) {
    ...
    const deduped = [...new Set(circuits.values())];
    if (deduped.length === 1) {
      const a = Number(node1.split(",")[0]);
      const b = Number(node2.split(",")[0]);
      console.log(a * b);
      break;
    }
  }
