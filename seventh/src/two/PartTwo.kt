...
import two.Hand.Companion.toHands
...
data class Hand(
    ...
    private fun valueOfHand(): HandValue {
        val perKind = cardsInHand
            .groupBy { it.value }

        val numJs = perKind
            .getOrDefault(Card.J.name, emptyList())
            .size

        val perKindNoJs = perKind.filterKeys { it != Card.J.name }

        return when {
            perKindNoJs.values.any { it.size + numJs == 5 } || numJs == 5 -> HandValue.FIVE_OF_A_KIND
            perKindNoJs.values.any { it.size + numJs == 4  } -> HandValue.FOUR_OF_A_KIND
            (perKind.values.any { it.size == 3 } && perKind.values.any { it.size == 2 }) ||
                    (numJs == 1 && perKindNoJs.values.filter { it.size == 2 }.size == 2) -> HandValue.FULL_HOUSE
            perKindNoJs.values.any { it.size + numJs == 3 } -> HandValue.THREE_OF_A_KIND
            numJs == 0 && hasTwoPairs(perKind) -> HandValue.TWO_PAIR
            perKindNoJs.values.any { it.size + numJs == 2 } -> HandValue.ONE_PAIR
